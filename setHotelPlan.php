...
?>
    ...
    <div class="searchPlace">
        <center><table>
            <tbody>
<?php
        $sql = "SELECT name,img,attractionId,googleid FROM attraction, attraction_type WHERE name LIKE"." '%".addslashes($query)."%' AND countryID = ".$_POST["country"]." AND attraction.attractionID = attraction_type.id AND type = 'lodging' GROUP BY attractionId";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
...
?>
            <script>
                $("#load").load("loadMoreHotel",{"query":"<?php echo $query.' '.$countryname?>",countryname: "<?php echo $countryname?>"});
            </script>
<?php
        }
    }
...
?>
    ...
    </table><button class="loadMoreHotel"><?php echo $setHotelPlan_text["loadmore"]?>></button></center>
</div>
